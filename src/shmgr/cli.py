...
Examples:
    # Load v1.* of the "foo.sh" shell library...
    eval "$(shmgr load foo.sh:1)"

    # Load v2.* of the "foo.sh" shell library. Exit with a non-zero exit code
    # if no version >=2.0.0 can be found...
    eval "$(shmgr load foo.sh:2)"

    # Load v2.* of the "foo.sh" shell library. Exit with a non-zero exit
    # code if no version >=2.1.0 can be found...
    eval "$(shmgr load foo.sh:2.1)"

    # Load v3.* of the "bar.sh" shell library. Exit with a non-zero exit code
    # if no version >=3.2.1 can be found...
    eval "$(shmgr load bar.sh:3.2.1)"
